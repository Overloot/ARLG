...
public class GameObject extends Entity {
    ...
    private String loot;
    //TODO: планируетс€ как параметр, в котором будет перечислен возможный лут.
    // ѕримерное содержимое: wood|95|ENTS|5
    // означает, что с веро€тность 95% выпадет древесина, а в 5% случа€х дерево станет Ёнтом.

    private String behavior; //TODO: поведение объекта. Ѕудет ли он нападать первым, враг или друг и т.д.

    private boolean destroyable; //TODO: разрушаемый объект или нет.
    // ѕланируетс€, что все объекты разрушаемые, но как разрушить портал или радугу?
    ...
    // крайне важный параметр. ќн убережет деревь€ от вырубки ножом дл€ намазывани€ масла =)

    // можно пойти дальше и прив€зать loot к используемуму дл€ убийства оружию. “.е. при ударе
    // файрболлом по дереву древесина не выпадет, при ударе топором будет древесина, а при использовании
    // кинжала можно будет наковыр€ть коры, но не древесины =)
    ...
    public GameObject(int id, String name, String path, int level) { //TODO: костыль дл€ BaseMonster
